...
Feature: As a MetalDesk user, I should be able to create a Buy limit Order for a specific Date/Time.
  ...
  Scenario Outline: To verify that all users can create a buy limit order for a Date/Time and that exchange balances update as expected
    ...
    And I validate the open order in the database with order details <contract_id>, "<direction>", <quantity>, "<order_type>" for the user "<data_set>" and order valid til "<order_valid_til>"
    And I exit the browser

    @pc
    Examples: PC
      | data_set | mode | hub | product | metal | contract_id | direction | quantity | order_type | distance | order_valid_til |

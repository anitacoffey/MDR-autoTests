...
Feature: As a MetalDesk user, I should be able to create a Buy limit Order for a Date/Time.
  ...
  Scenario Outline: To verify that all users can create a buy limit order for a Date/Time and that exchange balances update as expected
    ...
    And I place a "<direction>" limit order in the selected contract for a quantity of <quantity> to be placed for a specific date and time

...
And('I place a {string} market order for a quantity of {int}') do |direction, quantity|
  elements = TradePage.new
...
elements.order_quantity.click()
elements.order_quantity.send_keys(quantity)
...
) do |contract_id, direction, quantity, order_type, data_set|
  user = YamlLoader.user_info(data_set)

@fn_critical @tradescreen @7001 @limit @trade_behaviour
Feature: As a MetalDesk user, I should be able to create a Limit Order.
  ...
  Scenario Outline: To verify that all users can create a limit order and that results in a match
    ...
    @pc
    Examples: PC
      | data_set      | hub      | product         | metal | contract_id | direction | quantity | order_type | distance |
      | PC6_TestData1 | Brisbane | 10 oz Swiss Bar | gold  | 233         | buy       | 3        | limit      | -50      |

    @pb
    Examples: PB
      | data_set      | hub      | product         | metal  | contract_id | direction | quantity | order_type | distance |
      | PB3_TestData1 | Sydney   | 1 oz Pool       | silver | 206         | buy       | 5        | limit      | -2       |
      | PB3_TestData1 | Brisbane | 10 oz Swiss Bar | gold   | 233         | sell      | 2        | limit      | -50      |

@fn_critical @tradescreen @7002 @limit @trade_behaviour
Feature: As a MetalDesk user, I should be able to create a Limit Order.
  ...
  Scenario Outline: To verify that all users can create a limit order and this results in an open order in the database
    ...
    @pc
    Examples: PC
      | data_set      | hub      | product         | metal | contract_id | direction | quantity | order_type | distance | order_validity |
      | PC6_TestData1 | Brisbane | 10 oz Swiss Bar | gold  | 233         | buy       | 3        | limit      | 50       | GTC            |

    @pb
    Examples: PB
      | data_set      | hub      | product         | metal  | contract_id | direction | quantity | order_type | distance | order_validity |
      | PB3_TestData1 | Sydney   | 1 oz Pool       | silver | 206         | buy       | 5        | limit      | 2        | GTC            |
      | PB3_TestData1 | Brisbane | 10 oz Swiss Bar | gold   | 233         | sell      | 2        | limit      | 50       | GTC            |
